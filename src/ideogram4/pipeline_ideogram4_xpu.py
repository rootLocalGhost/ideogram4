...
def get_file_path(repo_or_dir: str, filename: str) -> str:
    """Returns a local file path if repo_or_dir is a local directory, otherwise downloads from Hub."""
    if os.path.isdir(repo_or_dir):
        local_path = os.path.join(repo_or_dir, os.path.normpath(filename))
        if os.path.exists(local_path):
            return local_path
        raise EntryNotFoundError(f"Local file not found: {local_path}")
    return hf_hub_download(repo_id=repo_or_dir, filename=filename)
...
def _load_subfolder_state_dict(
    ...
    try:
        ...
        single_path = get_file_path(repo_id, f"{prefix}{basename}.safetensors")
...
def _load_qwen3_vl(
    ...
    *,
    ...
    model_kwargs = (
        {"subfolder": text_encoder_subfolder} if text_encoder_subfolder else {}
    )
    ...
    cfg_path = get_file_path(
        repo_id,
        (
            f"{text_encoder_subfolder}/config.json"
...
def _build_transformer(
    ...
    if is_bnb4bit_state_dict(state_dict):
        if device.type != "cuda":
            raise ValueError(
                f"bnb 4-bit weights require a CUDA device, got device={device}"
            )
...
def _load_sharded_state_dict(
    ...
    index_path = get_file_path(repo_id, index_filename)
    ...
    for shard in shard_filenames:
        ...
        shard_path = get_file_path(repo_id, shard_repo_path)
...
def _load_indexed_or_single_state_dict(
    ...
    try:
        ...
        single_path = get_file_path(repo_id, single_filename)
        return load_file(single_path)
...
class Ideogram4Pipeline:
    ...
    def from_pretrained(
        ...
        *,
        ...
        # Do NOT load weights here to respect the strict sequential loading constraint.
        return cls(
            config=config,
            device=device,
            dtype=dtype,
    ...
    def _verify_prompts(
        ...
        for i, prompt in enumerate(prompts):
            ...
            if not issues:
                continue
            messages.append(
                f"caption verifier flagged prompt[{i}]:\n" + "\n".join(issues)
            )
    ...
    def __call__(
        ...
        *,
        ...
        # --- 1. Load Text Encoder, Execute, Purge ---
        print("Loading Text Encoder...")
        self.text_tokenizer, self.text_encoder = _load_qwen3_vl(
            self.config.weights_repo,
            self.device,
            self.dtype,
        ...
        # --- 2. Load Transformers, Execute, Purge ---
        print("Loading Transformers...")
        latent_dim = self.transformer_config.in_channels

        conditional_state_dict = _load_indexed_or_single_state_dict(
            self.config.weights_repo, self.config.conditional_index_filename
        )
        self.conditional_transformer = _build_transformer(
            self.transformer_config, conditional_state_dict, self.device, self.dtype
        )
        del conditional_state_dict
        gc.collect()

        unconditional_state_dict = _load_indexed_or_single_state_dict(
            self.config.weights_repo, self.config.unconditional_index_filename
        )
        self.unconditional_transformer = _build_transformer(
            self.transformer_config, unconditional_state_dict, self.device, self.dtype
        )
        del unconditional_state_dict
        gc.collect()
        torch.xpu.empty_cache()

        print(f"Running Diffusion Process ({num_steps} steps)...")
        ...
        for i in range(num_steps - 1, -1, -1):
            ...
            t = torch.full(
                (batch_size,), t_val, dtype=torch.float32, device=self.device
            )
        ...
        # Purge Transformers
        print("Unloading Transformers to free VRAM...")
        del self.conditional_transformer
        ...
        # --- 3. Load VAE Decoder, Execute, Purge ---
        print("Loading Autoencoder...")
        autoencoder_weights = get_file_path(
            self.config.weights_repo, self.config.autoencoder_filename
        )
        self.autoencoder = _load_autoencoder(
            autoencoder_weights, self.device, self.dtype
        )

        decoded = self._decode(z, grid_h=grid_h, grid_w=grid_w)

        # Purge Autoencoder
        print("Unloading Autoencoder to free VRAM...")
    ...
    def _decode(
        self, z: torch.Tensor, *, grid_h: int, grid_w: int
    ) -> list[Image.Image]:
